  ...
  inline XMLElement* save(MapSerializationHelper& helper)
    {
  	  ...
  	  mkfs->SetAttribute("size",int(sMeasurementKFs.size()));

  	  stringstream ss;
  	  //for(int i = 0; i < sMeasurementKFs.size(); i++)
  	  for(std::set<KeyFrame::Ptr>::iterator iter=sMeasurementKFs.begin(); iter!=sMeasurementKFs.end(); ++iter)
  	  {
  	  ...
  	  nrkfs->SetAttribute("size",int(sNeverRetryKFs.size()));

  	  stringstream ss2;
  	  //for(int i = 0; i < sNeverRetryKFs.size(); i++)
        for(std::set<KeyFrame::Ptr>::iterator iter=sNeverRetryKFs.begin(); iter!=sNeverRetryKFs.end(); ++iter)
  	  {
    ...
    inline void load(const XMLElement* mmdata, MapSerializationHelper& helper)
    {
  	  ...
  	  if(size>0)
  	  {
  		  ...
  		  //for(auto iter=sMeasurementKFs.begin(); iter!=sMeasurementKFs.end(); ++iter)
  		  {
  			  int kfid;
  			  ss >> kfid;
  			  sMeasurementKFs.insert(boost::shared_ptr<KeyFrame>(helper.GetKeyFrame(kfid)));
  		  }
  	  }
  	  ...
  	  if(size>0)
  	  {
  		  ...
  		  //for(auto iter=sNeverRetryKFs.begin(); iter!=sNeverRetryKFs.end(); ++iter)
  		  {
  			  int kfid;
  			  ss2 >> kfid;
  			  sNeverRetryKFs.insert(boost::shared_ptr<KeyFrame>(helper.GetKeyFrame(kfid)));
  		  }
  	  }
