...
struct KeyFrame
{
  ...
  /*
  XMLElement* save(MapSerializationHelper& helper, int kfid)
  {
	  XMLDocument* doc = helper.GetXMLDocument();
	  ...
	  for(std::map<boost::shared_ptr<MapPoint>, Measurement>::iterator outer_iter=mMeasurements.begin(); outer_iter!=mMeasurements.end(); ++outer_iter)
	  {
  ...
  void load(const XMLElement* keyframe, MapSerializationHelper& helper, int kfid)
  {
	...
	if(measurements!=NULL)
	{
		...
		while(measurement!=NULL)
		{
			...
			mMeasurements[boost::shared_ptr<MapPoint>(helper.GetMapPoint(mpid))] = m;
