...
void MapMaker::run()
{
  ...
  const PtamParamsConfig& pPars = PtamParameters::varparams();
...
bool MapMaker::InitFromLoad()
{
	 ...
	 */
	cout<<"map point use count: "<<mMap.vpPoints[0].use_count()<<endl;
	return true;
}
...
//Weiss{
	...
	{
		cout<<"Too few matches to init."<<endl;
		return false;
	}
...
//    mMap.Reset();
    ...
    if(i==0) cout<<"map point use count: "<<p.use_count()<<endl;
...
#ifdef KF_REPROJ
    if(i==0) cout<<"map point use count: "<<p.use_count()<<endl;
    pkFirst->AddKeyMapPoint(p);	//pointer count +6
    if(i==0) cout<<"map point use count: "<<p.use_count()<<endl;
    pkSecond->AddKeyMapPoint(p);//pointer count +6
    if(i==0) cout<<"map point use count: "<<p.use_count()<<endl;
#endif
    //}
  }
...
//Weiss{
	...
	{
		cout<<"Something is seriously wrong with the first KF."<<endl;
		return false;
	}
	if(!RefreshSceneDepth(pkSecond))
	{
		cout<<"Something is seriously wrong with the second KF."<<endl;
		return false;
	}
  ...
  const PtamParamsConfig& pParams = PtamParameters::varparams();
  ...
  if(!addedsome)
	{
		cout<<"Could not add any map points on any level - abort init."<<endl;
		return false;
	}
  //}
  ...
  {
    cout<<"Initial map rejected because of too large point variance. Point sigma: " << ((pkFirst->dSceneDepthSigma+pkSecond->dSceneDepthSigma)/2.0)<<endl;
    return false;
  }
...
void MapMaker::SaveMap()
{

	cout<<"save map called"<<endl;
	string mapDir = "save";
	...
	cout<<"request continue success"<<endl;


	//TODO delete files in folder

	MapSerializationHelper helper(mapDir);
	...
	cout<<"camera saved"<<endl;

	//save map
	XMLElement *keyframes = doc->NewElement("KeyFrames");
	for(int i = 0; i < mMap.vpKeyFrames.size(); i++){
		keyframes->InsertEndChild(mMap.vpKeyFrames[i]->save(helper,helper.GetKeyFrameID(mMap.vpKeyFrames[i])));
	}
	...
	for(int i = 0; i < mMap.vpPoints.size(); i++)
		points->InsertEndChild(mMap.vpPoints[i]->save(helper,helper.GetMapPointID(mMap.vpPoints[i])));
	...
	for(int i = 0; i < mMap.vpPointsTrash.size(); i++)
		trashpoints->InsertEndChild(mMap.vpPointsTrash[i]->save(helper,helper.GetMapPointID(mMap.vpPoints[i])));
	map->InsertEndChild(trashpoints);

	cout<<"TrashPoints saved"<<endl;

	//save mapmaker stuff
	XMLElement *newkeyframes = doc->NewElement("NewKeyFrames");
	for(int i = 0; i < mvpKeyFrameQueue.size(); i++)
		newkeyframes->InsertEndChild(mvpKeyFrameQueue[i]->save(helper,helper.GetKeyFrameID(mvpKeyFrameQueue[i])));
	map->InsertEndChild(newkeyframes);
	cout<<"NewKeyFrames saved"<<endl;

	XMLElement *failures = doc->NewElement("FailureQueue");
	for(std::vector<std::pair<KeyFrame::Ptr, boost::shared_ptr<MapPoint> > >::iterator iter=mvFailureQueue.begin(); iter!=mvFailureQueue.end(); ++iter)
	{
...
void MapMaker::LoadMap()
{
	...
	while(mappoint!=NULL)
	{
		...
		if(first){
			cout<<"mp use count: "<<mMap.vpPoints[0].use_count()<<endl;
			first=false;
		}
	}
	...
	while(keyframe!=NULL)
	{
		...
		helper.GetKeyFrame(kfid)->load(keyframe,helper,kfid);
		keyframe = keyframe->NextSiblingElement("KeyFrame");
	}
	keyframes = map->FirstChildElement("NewKeyFrames");
	keyframe = keyframes->FirstChildElement("KeyFrame");
	...
	while(failure!=NULL)
	{
		...
		KeyFrame::Ptr kf = helper.GetKeyFrame(kfid);
		MapPoint::Ptr mp = helper.GetMapPoint(mpid);
		mvFailureQueue.push_back(std::pair<KeyFrame::Ptr, MapPoint::Ptr >(kf, mp));
	...
	while(newmp!=NULL)
	{
		...
		mqNewQueue.push(MapPoint::Ptr(helper.GetMapPoint(mpid)));
	...
	//ATANCamera test;
	mCamera.load(map->FirstChildElement("Camera"),helper);
...
void MapMaker::LoadMap()
{
	...
	while(keyframe!=NULL)
	{
		...
		helper.RegisterKeyFrame(kf); //TODO maybe register keyframes and mappoints with saved IDs (instead of assuming to be in same order as during save) to allow easy manual map editing?
		mMap.vpKeyFrames.push_back(KeyFrame::Ptr(kf));
		keyframe = keyframe->NextSiblingElement("KeyFrame");
	}
	...
	while(keyframe!=NULL)
	{
		KeyFrame* kf = new KeyFrame();
		helper.RegisterKeyFrame(kf);
		mvpKeyFrameQueue.push_back(KeyFrame::Ptr(kf));
		keyframe = keyframe->NextSiblingElement("KeyFrame");
	}
	...
	while(mappoint!=NULL)
	{
		...
		helper.RegisterMapPoint(mp);
		mMap.vpPoints.push_back(MapPoint::Ptr(mp));
		mappoint = mappoint->NextSiblingElement("MapPoint");
	}
	...
	while(mappoint!=NULL)
	{
		MapPoint* mp = new MapPoint();
		helper.RegisterMapPoint(mp);
		mMap.vpPointsTrash.push_back(MapPoint::Ptr(mp));
		mappoint = mappoint->NextSiblingElement("MapPoint");
	}
